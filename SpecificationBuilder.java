package com.funteaching.educationmanage.Utils;
...
public class SpecificationBuilder {
    ...
    public Predicate getPredicate(Predicate p, Root<?> root, CriteriaBuilder cb, String path, String op, Object value) {
        ...
        if (op.equals("=")) {
            ...
            throw new java.lang.Error("illegal op" + op);
        }
    ...
    private Predicate getIn(Root root, CriteriaBuilder cb, String path, Object value, boolean in) {
        ...
        if (value instanceof List) {
            ...
            if (item instanceof Long) {
                List<Long> inset = (List<Long>) value;
                predicate = tmp.in(inset);
            }
            if (item instanceof Integer) {
                List<Integer> inset = (List<Integer>) value;
                predicate = tmp.in(inset);
            }
            if (item instanceof String) {
                List<String> inset = (List<String>) value;
                predicate = tmp.in(inset);
            }
        } else {
